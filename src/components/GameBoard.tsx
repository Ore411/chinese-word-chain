              ...
              {members.length > 0 ? (
                <>
                  ...
                  {members.map(m => (
                    <span
                      key={m}
                      className={`px-1.5 py-0.5 rounded text-xs font-mono ${m === initial ? 'bg-emerald-700 text-emerald-200' : 'bg-slate-700 text-slate-300'}`}
                    >
                      {m}
                    </span>
                  ))}
                ...
                <span className="text-slate-500 text-xs">vowel-start only (no initial)</span>
              )}
            </div>

            {/* Compatible finals row */}
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {compatFinals.map(cf => (
                <span
                  ...
                  className={`px-1.5 py-0.5 rounded text-xs font-mono ${
                    cf.type === 'exact' ? 'bg-emerald-700 text-emerald-200' :
                    cf.type === 'strong' ? 'bg-sky-800 text-sky-200' :
